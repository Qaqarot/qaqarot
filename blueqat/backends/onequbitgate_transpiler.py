...
class OneQubitGateCompactionTranspiler(Backend):
    ...
    def _run_inner(self, gates, operations: List[Operation], singlemats: List[np.array],
            n_qubits: int, mat1_decomposer: Callable[[OneQubitGate], List[Operation]]):
        for gate in gates:
    ...
    def run(self, gates, n_qubits: int, *,
            mat1_decomposer: Optional[Callable[[OneQubitGate], List[Operation]]] = None, **kwargs) -> Circuit:
        """
        Args:
        ...
        self._run_inner(gates, operations, singlemats, n_qubits, mat1_decomposer)
